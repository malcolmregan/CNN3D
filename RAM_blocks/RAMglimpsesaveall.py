...
for CLASS in range(0,10):
    ...
    test_set = H5PYDataset('RAM_CLASS{0}.hdf5'.format(CLASS), which_sets=('test',))
    handle = test_set.open()
    model_idx = 0

    for model_idx in range(0,test_set.num_examples):
        print('\tExample {}'.format(model_idx))
...
################################################################################
	...
	#plt.show(True)
	plt.suptitle('RAM Glimpses, RAM+CPPN Class {0}, Example {1}'.format(CLASS, model_idx), size=25)
        plt.savefig('images/RAMglimpse/RAM_Class{0}_Example{1}.png'.format(CLASS,model_idx))
        plt.close()
